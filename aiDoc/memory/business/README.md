...
- [2026-08-12 股票热榜开盘时段每 5 分钟同步](./2026-08-12_stock_hot_5min_sync.md) — 热榜抓取 cron 改 `*/5 9-15 * * mon-fri` + 任务内交易时段守卫（9:30-11:30、13:00-15:00，非时段跳过）；顺带修复 APScheduler 星期坑（数字 `1-5`=周二至周六），三个收盘行情任务改 `mon-fri`，补回周一收盘数据；同日快照仍只保留首次抓取
